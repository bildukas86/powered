...
public class AdminEvents implements IAdminCommandHandler
{
	...
	private static void listEvents(L2PcInstance activeChar)
	{
		...
		for (int id : events.keySet())
		{
			count++;
			//@formatter:off
			tb.append("<center><table width=270 " + (count % 2 == 1 ? "" : "bgcolor=5A5A5A") + "><tr>" +
					"<td width=145>" + events.get(id) + "</td>" +
					"<td width=60>" + (EventManager.getInstance().isEnabled(id) 
						? "<a action=\"bypass -h admin_enableEvent " + id + " 0\">Disable</a>" : "<a action=\"bypass -h admin_enableEvent " + id + " 1\">Enable</a>") + "</td>" +
					"<td width=65><a action=\"bypass -h admin_configureEvent " + id + "\">Configure</a></td></tr></table>");
			//@formatter:on
		}
	...
	private static void showMainPage(L2PcInstance activeChar, int eventId)
	{
